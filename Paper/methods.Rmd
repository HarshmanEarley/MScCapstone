...
During tuning, a number of other parameters are grid searched through to give results of a number different style models:
...
* **Regularization:** l2 regularization is available on each layer, adding the squared magnitude of coefficients as penalty to the loss function. This is a feature to help generalise the model and stop over fitting. Values of 0, 0.001 and 0.0001 are used to tune.      
* **Normalization:** Batch normalisation is available on each layer. When activated output of each layer is scaled and centered before being passed to subsequent layers.     
