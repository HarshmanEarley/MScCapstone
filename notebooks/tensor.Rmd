...
```{r}
asd = read_parquet("/Users/root1/Documents/amex-default-prediction/parquet/train.parquet")
```


```{r}
format(object.size(qwe), units = "Gb", standard = "auto", digits = 1L)
```


```{r}
0.001*sqrt(5500000/100)
```
...
```{r}
qwe = as.data.frame(sampled_data)
```

```{r}
qwe %>%  map_batches(~ sample_frac(as.data.frame(.), 0.01))
```
