...
```{r}
targetPrediction = model_neuralNetwork(getFilePath("data_lastPerCustomerID",".parquet"), tuning = FALSE, bestModelFlags = bestModelFlags)
```
...
```{r, eval = FALSE}
amex_metric(targetPrediction$target, targetPrediction$prediction)
```
```{r}
par(mfrow = c(1,2))
hist(targetPrediction$target)
hist(ifelse(targetPrediction$prediction > 0.5, 1,0)[,1], )
```
